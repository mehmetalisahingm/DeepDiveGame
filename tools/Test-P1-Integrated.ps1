param([ValidateSet(1,2,4)][int]$Players = 4, [int]$Port = 18777, [switch]$Capture, [switch]$Hunt, [switch]$Record, [switch]$Event, [switch]$Town, [switch]$Boat, [switch]$Trip)
...
try {
    ...
    $p1Deadline = (Get-Date).AddSeconds($(if ($Town) { 105 } elseif ($Event) { 150 } elseif ($Trip) { 175 } elseif ($Boat) { 95 } elseif ($Hunt -or $Record) { 90 } else { 60 }))
