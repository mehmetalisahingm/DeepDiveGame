...
| P3.2 Kumsal, NPC, ilk görev ve ayrı kamera | Mert kasaba/NPC/ekonomi/kayıt; Utku kıyı; Mehmet etkileşim | **DEVAM** — Mert kısmı (#62) PR'da: üç NPC, bekleyen teslim + NPC'de tek seferlik ödeme, ayrı kamera, sandal parça durumu, save v2; NPC'ler #61 kumsalıyla birlikte DiveTestArea'da (`Beach_TownGate` yanında); EditMode 550/550 ve gerçek iki süreçli smoke (`-Town`, `-Record`, `-Event`, `-Hunt`, temel) yerelde geçti. **Kısıt:** smoke'lar oyuncuyu Return'de rampa tepesine host'tan taşır (`townLandingTeleported`); yüzerek kumsala çıkış şu an mümkün değil (bkz. CONTRACTS P3.2-C notu) ve doğrulanmış sayılmaz. Utku kumsalı/parça yerleşimi (#61), Mehmet'in NetworkDiver/co-op kabulü ve ayrı bilgisayar/internet smoke'u açık; guest'in satın aldığı kamera yeniden açmada saklanmaz (D06) |
