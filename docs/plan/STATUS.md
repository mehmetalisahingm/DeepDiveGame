...
| P3.2 Kumsal, NPC, ilk görev ve ayrı kamera | Mert kasaba/NPC/ekonomi/kayıt; Utku kıyı; Mehmet etkileşim | **DEVAM** — Mert kısmı (#62) PR'da: üç NPC, bekleyen teslim + NPC'de tek seferlik ödeme, ayrı kamera, sandal parça durumu, save v2; PR #65 sonrası entegrasyonla birleşik hâlde EditMode 492/492 ve gerçek iki süreçli smoke (temel, `-Hunt`) yerelde geçti. NPC anchor'ları PrepArea'da kaldığı için `-Town`/`-Record`/`-Event` smoke'ları NPC'ler #61 kumsalı sonrası DiveTestArea'ya taşınana kadar PASS sayılmaz (önceki PASS, PR #65 öncesi sahne modeline aitti). Utku kumsalı/parça yerleşimi (#61), Mehmet'in NetworkDiver/co-op kabulü ve ayrı bilgisayar/internet smoke'u açık; guest'in satın aldığı kamera yeniden açmada saklanmaz (D06) |
