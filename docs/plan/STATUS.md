...
| P3.1 İnsan, yürüme/yüzme, elde kamera/zıpkın | Mehmet entegrasyon; Mert insan/animasyon/ekipman seti; Utku su geçişi | **DEVAM** — 23 Eylül insan/kol/kamera kaynağı ve oyuncu sunumu çalışma dalında; özel Mixamo indirmesi gerekmiyor. [Teslim/test kaydı](../reports/P3-HUMAN-VISUAL-REPAIR.md). Ortak görsel kabul ve doğal yüzme/sanat iyileştirmesi açık |
| P3.2 Kumsal, NPC, ilk görev ve ayrı kamera | Mert kasaba/NPC/ekonomi/kayıt; Utku kıyı; Mehmet etkileşim | **TAMAMLANDI** (#60/#61/#62 completed, 2026-09-23) — üç NPC kumsalda, bekleyen teslim + NPC'de tek seferlik ödeme, ayrı kamera, sandal onarımı gerçek E-pickup ile kanıtlandı (#72), su→kumsal çıkışı ve P3.1 rampası düzeltildi (#71), E-pickup'ın SwimVolume ile çakışması giderildi (#74). Gerçek iki süreçli smoke (`-Town`, `-Record`, `-Event`, `-Hunt`, `-Boat`, temel) kısayolsuz PASS |
| P3.3 Sandal, harita ve gidiş/dönüş | Mehmet hareket/koltuk/konum; Utku rota/koordinat; Mert onarım/harita UI/kayıt | **DEVAM** (kapı 2026-09-23 açıldı) — seam freeze önerisi #66'da kabul gördü (`boat-1`/`route-near-1`/`seat-0..3`, `Docked→Outbound→Anchored→Inbound→Docked`). Mert'in bağımsız kısmı PR #77'de: `BoatTripState`/`BoatBoarding`/`BoatRouteProgress` sözleşmeleri, host-authoritative `BoatTripManager`, kayıt/save-load (yeni alan yok — gerekçe CONTRACTS P3.3-C notunda), `BoatMapPresenter` harita hesaplaması; EditMode 567/567 (bkz. `docs/plan/CONTRACTS.md` P3.3-C notu). Mehmet'in #75'i ve Utku'nun #76'sı henüz kod içermiyor; koltuk/hareket/dock/rota/gerçek smoke bunlar gelince birlikte yapılacak |
