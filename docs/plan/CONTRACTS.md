...
1. **Üç hizmet ve sahne.** `TownServiceCatalog`: `shop-equipment` (EquipmentShop), `buyer-fish` (FishBuyer), `buyer-recording` (RecordingBuyer); anchor id'leri `anchor-shop-equipment` / `anchor-buyer-fish` / `anchor-buyer-recording`, mesafe 3 m. `ServicePointAnchor` NPC'leri **PrepArea** sahnesindedir: `SessionNetworkAdapter` Lobby/Prep/Return'de PrepArea, yalnız Dive'da DiveTestArea yükler ve dalışta hizmet etkileşimi zaten reddedilir. Sahne `DeepDive/P3.2/Prep area: town service NPCs` menüsüyle (idempotent) kurulur; NPC'ler ilkel yer tutucudur, gerçek NPC sanatı sonraki geçiştir. Sahnedeki tanım katalogla id **ve tür** olarak eşleşmezse (`TownServiceCatalog.Matches`) işlem reddedilir.
