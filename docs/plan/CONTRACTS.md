...
1. **Üç hizmet ve sahne.** `TownServiceCatalog`: `shop-equipment` (EquipmentShop), `buyer-fish` (FishBuyer), `buyer-recording` (RecordingBuyer); anchor id'leri `anchor-shop-equipment` / `anchor-buyer-fish` / `anchor-buyer-recording`, mesafe 3 m. `ServicePointAnchor` NPC'leri **geçici olarak PrepArea'dadır ve final merge'e girmeyecektir**: PR #65'ten beri `SessionNetworkAdapter.SceneForPhase` yalnız Lobby için PrepArea, Prep/Dive/Return için DiveTestArea (kıyı dünyası) yükler; hizmet kapısı sahneye değil faza bağlıdır (Dive dışında açık), yani NPC'ler #61 kumsalı geldikten sonra DiveTestArea'ya taşınacaktır (anchor konumunu World/Utku belirler; id'ler ve katalog değişmez). Bugünkü PrepArea kurulumu `DeepDive/P3.2/Prep area: town service NPCs` menüsüyle (idempotent) yapılır; NPC'ler ilkel yer tutucudur, gerçek NPC sanatı sonraki geçiştir. Sahnedeki tanım katalogla id **ve tür** olarak eşleşmezse (`TownServiceCatalog.Matches`) işlem reddedilir.
