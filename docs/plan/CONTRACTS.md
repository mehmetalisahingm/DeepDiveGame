...
Bu bölüm taslak değil, kodda olanı yazar; doğrulanan/doğrulanmayan ayrımı PR açıklamasındadır. Şekiller `Assets/DeepDive/Core/P3BoatTripContracts.cs`, kurallar `Assets/DeepDive/Trip/`. Mehmet'in #75'i (koltuk/binme/hareket) ve Utku'nun #76'sı (iskele/rota/world↔map) bu commit'te henüz yok; bu not yalnız Mert'in bağımsız tamamladığı katmanı kapsar.
...
5. **Bilerek yapılmayanlar.** `BoatTripPlayerSync` (ağ senkron aynası) yazıldı ama NetworkDiver prefab'ına/Composition'a bağlanmadı — Mehmet'in oyuncu/prefab otoritesi alanı. Gerçek dock/koltuk/rota dünya nesneleri veya bunların yer tutucusu kurulmadı — Utku'nun alanı, "final" gibi davranacak bir taklit yapılmadı. Gerçek iki süreçli board/sail/return smoke #75+#76+bu iş integration'da buluşunca yapılacak.
