...
### P3.2-C uygulama notu — NPC hizmetleri, bekleyen teslim, kamera, sandal parçaları (#62)

Bu bölüm taslak değil, kodda olanı yazar; doğrulanan/doğrulanmayan ayrımı PR açıklamasındadır. Şekiller `Assets/DeepDive/Core/P3TownContracts.cs`, kurallar `Assets/DeepDive/Economy/`.

1. **Üç hizmet ve sahne.** `TownServiceCatalog`: `shop-equipment` (EquipmentShop), `buyer-fish` (FishBuyer), `buyer-recording` (RecordingBuyer); anchor id'leri `anchor-shop-equipment` / `anchor-buyer-fish` / `anchor-buyer-recording`, mesafe 3 m. `ServicePointAnchor` NPC'leri **DiveTestArea kumsalındadır** (PR #65'ten beri `SessionNetworkAdapter.SceneForPhase` yalnız Lobby için PrepArea, Prep/Dive/Return için DiveTestArea yükler; hizmet kapısı sahneye değil faza bağlıdır, Dive dışında açık). Üçü de kumsal şeridinde, Utku'nun `Beach_TownGate` işaretinin 3 m doğusunda, 2,4 m aralıkla ve suya (+z) bakar; konum kapıdan setup sırasında türetilir, sabit koordinat değildir. PrepArea'da hiç anchor kalmadı. Sahne `DeepDive/P3.2/Dive test area: town service NPCs` menüsüyle (idempotent, yalnız ekleme) kurulur; menü `Beach_TownGate` yoksa çalışmayı reddeder. NPC'ler ilkel yer tutucudur, gerçek NPC sanatı sonraki geçiştir. **Bilinen engel:** yüzen oyuncu kumsala kendi başına çıkamıyor (Surface modunda yukarı girdi kesilir, ayak en çok 6,30'a çıkar; sığ rampanın ayağı 6,80-7,21). Smoke'lar bu yüzden oyuncuları Return'de rampa tepesine `NetworkPlayer.Teleport` ile koyar ve bunu `townLandingTeleported` alanıyla raporlar; su→kara çıkışı doğrulanmış sayılmaz. Sahnedeki tanım katalogla id **ve tür** olarak eşleşmezse (`TownServiceCatalog.Matches`) işlem reddedilir.
2. **Etkileşim.** Mehmet'in seam'i (`ServiceInteractionAuthority`) mesafe/nişan/sahiplik doğrular; `TownServiceHandler` ekonomi tarafını uygular. Balık alıcısı / kayıt alıcısı: etkileşen oyuncunun elindeki tüm bekleyen ürünleri teslim alır ve öder. Ekipman dükkânı: oyuncuya menzil içinde açık bir **dükkân oturumu** verir; `EconomyPurchaseAuthority` (satın alma RPC'si) oturum yoksa veya oyuncu menzilden çıktıysa `NotAtShop` döner. Dalış başlayınca/oyuncu uzaklaşınca oturum kapanır.
3. **PendingTurnInState.** Dalış sonu artık para vermez: `EconomyManager.QueuePreservedCatches` güvenli dönen avı, `TryQueueRecordingTurnIn` (eski `TryRewardRecording`) kaydı bekleyen ürün yapar; eski doğrudan ödeme yolu kaldırıldı, ikisi aynı anda açık değildir. Kayıt fiyat için gereken alanları (tür/kalite/ağırlık) kopyalar; çünkü envanter bir sonraki dalışta av kayıtlarını unutur. Taşıyıcı oyuncu ya da **ortak emanet**: taşıyıcı yalnız kendi ürününü, emanet herkes tarafından teslim edilebilir; ikinci teslim `NothingToTurnIn` alır, tek ödeme olur.
4. **İdempotency.** Ürün başına tek kimlik (`SoldCaptureIds` / `PaidRecordingIds`); ayrıca `(playerId, requestId, işlem türü)` yeniden oynatma önbelleği — işlem türü anahtarda olduğundan satın alma RPC'sinin ve etkileşimin bağımsız sayaçları çakışmaz. Ödeme + ürün tüketimi + save tek işlemdir; disk hatasında hepsi geri alınır, `SaveFailed` önbelleğe alınmaz (aynı requestId sonra yeniden denenebilir). Ortak para eksiye düşmez.
5. **Temel kamera.** `camera-basic` (yuva `camera`, seviye 1, fiyat 150) ayrı satın alınan üründür, varsayılan sahiplik yoktur. v1'de ekipman "instance" = oyuncu başına tek SKU (`AlreadyProcessed` ikinci kopyayı engeller); Mehmet `LoadoutState` + `DiverEquipmentRules.OwnsEquipmentSlot(...,"camera")` ile çözer, başka oyuncunun loadout'u uygulanmaz.
6. **Sandal onarımı.** Sabit tekne `boat-1`, üç parça: `boat-part-hull`, `boat-part-engine`, `boat-part-fuel-tank` (**Utku'nun dünya anchor'ları tam bu kimlikleri kullanmalı**). Ücretsiz parça: World/etkileşim katmanı geçerliliği doğruladıktan sonra `BoatPartClaim.TryClaimFound(player, partId, requestId)` çağırır; satın alma: dükkândan parça başına 120. İkisi aynı ilerleme durumunu ilerletir; her parça en çok bir kez katkı olur (bulunmuş parça sonradan satın alınmaz/ücretlendirilmez), katkı parçayı tüketir — satılamaz. 0 parça Broken, 1–2 InProgress, 3 Repaired. Sandal hareketi/koltuk/rota P3.3'tür.
7. **Kayıt şeması v2.** `EconomySaveData` v2: bekleyen ürünler + tamamlanmış parça kimlikleri. v1 dosyalar yüklenir (bekleyen boş, sandal bozuk); gelecekteki/bilinmeyen sürüm reddedilir ve mevcut durum korunur. Diskteki ödenmiş kimlik asla bekleyen olarak geri gelmez. Host dışındaki taşıyıcıların ürünü kayıtta ortak emanete çevrilir (guest bağlantı ID'si kalıcı kimlik değildir).
8. **Bilinen sınırlar (karar gerektirir).** (a) Bekleyen ürün çantada tutulmaz: kapasite tüketmez ve sonraki dalışa risk olarak taşınmaz — yukarıdaki taslaktaki "taşınan satılmamış av kapasite tüketir" maddesinden bilinçli sadeleştirme, ekiple netleşmeli. (b) Kayıt yalnız host'un loadout'unu saklar (D06): guest'in satın aldığı kamera oturum içinde geçerlidir, yeniden açmada geri gelmez. (c) Dükkân arayüzü IMGUI yer tutucudur.

### P3.3-C uygulama notu — sefer state machine, kayıt, harita (#66)

Bu bölüm taslak değil, kodda olanı yazar; doğrulanan/doğrulanmayan ayrımı [P3.3 runtime kabul kaydındadır](../reports/P3-BOAT-RUNTIME-ACCEPTANCE.md). Şekiller `Assets/DeepDive/Core/P3BoatTripContracts.cs`, kurallar `Assets/DeepDive/Trip/`, host hareketi `Assets/DeepDive/Network/NetworkBoatController.cs` ve bağlantı `Assets/DeepDive/Composition/BoatTripNetworkBinding.cs` içindedir. #75/#76/#66 entegrasyonu `codex/p3-integration`'da birleşmiştir; oyuncu-facing harita UI'sı ve dört süreçli/ayrı bilgisayar kabulü P3.4 kapsamındadır.

1. **Sabit kimlikler.** `boat-1` (#62'den), tek yakın rota `route-near-1`, dört koltuk `seat-0`..`seat-3`. Dock/anchor/route dünya verisi Utku'nundur; Core yalnız stabil anchor **id**'lerini tutar (`DiveRouteDefinition.DepartureDockAnchor`/`AnchorPointAnchor`), `ServicePointDefinition.WorldAnchor` ile aynı gerekçeyle — Core UnityEngine'den bağımsız kalır.
2. **Faz makinesi.** `Docked → Outbound → Anchored → Inbound → Docked`. Otorite `BoatTripManager`'dadır (host-only); ikinci bir otorite açılmaz. `BoatBoarding` (Mehmet'in koltuk doğrulaması → `TryBoard`/`TryDisembark`) ve `BoatRouteProgress` (onun hareket kodu fiziksel varışta → `ReportArrival`) `BoatPartClaim`'in Bind/Unbind deseniyle birebir aynı. Binme/inme `Docked` ve `Anchored`'da açık (demirlemede dalıp geri binmek sefer partisini bozmaz, yalnız koltuğu boşaltır); hareket sırasında kapalı. Yalnız sefer sorumlusu başlatır/geri çağırır; kopmada sahiplik bağlı bir yolcuya devredilir, kimse kalmazsa boşalır. Boş demirlemiş sandal kendini otomatik geri çağırır.
3. **Kayıt/save-load.** Yeni bir save alanı **yok**. `BoatTripState` hiç serialize edilmez — CONTRACTS zaten "hareket hâlindeki sefer restore edilmez" diyor, ve v1'in tek rotasının gerçek kilidi `BoatRepairState.Status==Repaired`, ki bu #62'nin `EconomySaveData` (şema v2, `BoatPartIds`) ile zaten kalıcı. `BoatTripManager` taze bir örnekte zaten `Docked`/koltuklar boş durumundadır; `ResetToDocked()` bunu açık/test edilebilir kılar. Bilinçli karar: ikinci bir rota veya sefer ödülü (P4.3) gelmeden yeni save alanı açılmayacak — gelirse burası güncellenir.
4. **Harita.** `BoatMapPresenter.BuildIcons(tripState, dock, anchor?, liveBoat?)` saf hesaplama: `Docked`'ta sandal dock'ta, `Anchored`'ta anchor'da (Utku'nun verisi varsa), `Outbound`/`Inbound`'da yalnız Mehmet'in onayladığı canlı konum varsa gösterilir — hiçbiri yoksa ikon **gizlenir**, asla enterpolasyonla tahmin edilmez (CONTRACTS: "Mehmet onaylı oyuncu/aktif tekne konumu"). Girdi/çıktı zaten normalize map-space (`MapIcon.MapX/MapZ`); world↔map dönüşümü tamamen Utku'da kalır, UI'da tekrarlanmaz.
5. **Bilerek yapılmayanlar.** Oyuncu-facing harita ekranı ve dört süreçli/ayrı bilgisayar/internet kabulü bu notun dışında kalır. `BoatTripPlayerSync` NetworkDiver prefab'ına ve Composition'a bağlıdır; gerçek dock/koltuk/rota dünya nesneleri sahnededir. Gerçek iki süreçli board/sail/return smoke geçti; kanıt [P3.3 runtime kabul kaydındadır](../reports/P3-BOAT-RUNTIME-ACCEPTANCE.md).

### P3.3-B uygulama notu — iskele/rota/world↔map (#76)

Bu bölüm taslak değil, kodda olanı yazar. Yukarıdaki P3.3-C notunun "Utku'nun #76'sı bu commit'te henüz yok" boşluğunu kapatır. Dünya verisi `Assets/DeepDive/World/Map/` ve `Assets/DeepDive/World/Boat/`, sahne yazımı `Assets/DeepDive/World/Editor/DiveTestAreaRouteSetup.cs`, sahne `DiveTestArea`. Mehmet'in #75'i (koltuk/binme/hareket) bu commit'te hâlâ yok; bu not yalnız Utku'nun teslim ettiği katmanı kapsar.

1. **Sabit kimlikler.** `route-near-1`, `dock-town-1`, `anchor-near-1` — hepsi Core'un sabitlerinden okunur, ikinci kez literal yazılmaz. `region-near-1` bilinçli olarak Core'da **değildir**: P3.3'te tek bölge var, hiçbir şey onu kalıcılaştırmaz ve üç kişinin paylaştığı sözleşmeye bir hata ayıklama etiketi eklemek kimsenin ihtiyacı olmayan bir değişiklik olurdu.
2. **World↔map dönüşümü.** Aritmetik `DiveRegionBounds` (saf `readonly struct`, UnityEngine sahne sorgusu yok); sahnedeki karşılığı `DiveRegionField` — yalnız yazılmış extent'leri tutan ince kabuk. **Presenter değildir:** `MapIcon` üretmez, UI/Composition/netcode yüzeyi yoktur. Sahnede tam bir tane bulunur; ikincisini `TryFind` seçmek yerine reddeder. Bölge suyun tüm footprint'ini kapsar, dışındaki nokta **clamp edilmez, reddedilir** (yanlış cevap cevapsızlıktan kötüdür). Harita ekranı ve ikon otoritesi Mert'te kalır; dönüşüm UI'da tekrarlanmaz.
3. **Rota ve demirler.** `DiveRoutePath` bir rotanın dünya geometrisidir: kardeş sırası rota sırasıdır, noktalar world-space'tir, `TryFind` id'yi tam eşler ve "sahnedeki tek rota"ya düşmez. `RouteAnchor` konum + kimlik + binme noktasıdır; sefer fazı taşımaz (o Mert'in `BoatTripState`'i), hareket etmez (o Mehmet'in mover'ı). İkisi de authored scenery olduğundan `NetworkObject`/`NetworkBehaviour` taşımaz.
4. **Sahne.** `DeepDive/P3.3/Dive test area` menüsü `Dock_Town`, `Dock_Town_Anchor`, `Route_Near` (`WP_0..WP_2`), `Anchor_Near` + `Boarding` ve `DiveRegion` nesnelerini yazar. Hiçbir şeyi silmez, eksiği yaratıp var olanı yerinde ayarlar ve `ApplyAndVerify` ikinci koşunun dosyayı byte-aynı ürettiğini kanıtlar. On yazar-zamanı guard'ı var. İki şeyi bilinçle yapmaz: iskele **ikinci bir güvenli dönüş üretmez** (dönüş pad'inin 0.25 m kuzeyinde biter; `SafeReturnZone` tek otorite kalır) ve demirlemede kara yapmaz (gerçek su, altında 8 m). `WaterField`/`SwimVolume` açılmaz; su sınıflandırması yalnız okunur.
5. **Core binding.** `DiveRoutePath.ToDefinition()` Core'un `DiveRouteDefinition`'ını üretir: sabit id'ler + Mehmet'in donmuş nominali `OutboundSeconds=8`, `InboundSeconds=8`. Core **okunur ve construct edilir, değiştirilmez**. World tarafında hız/süre hesabı yoktur — mover kendi hızını yürüdüğü waypoint'lerden türetir; rota uzunluğundan saniye üretmek "tekne ne kadar hızlı" sorusuna ikinci bir cevap olurdu. Bilinmeyen veya yarım yazılmış rota tarif edilmez, reddedilir. Anchor id'leri path başına serialize edilmedi: alan eklemek sözleşmenin zaten sabitlediği değerleri yazmak için sahneyi yeniden kaydetmeyi gerektirirdi — ikinci rota (P4.3) bunu gerektiren fazdır.
